...
def GestisciLogin():
    ...
    if content_type == 'application/json':
        jsonReq = request.jsons
        UsernameInseritoDalClient = jsonReq["username"]
        if  UsernameInseritoDalClient in utenti:
            PasswordInseritaDalClient = jsonReq["password"]
            if  PasswordInseritaDalClient == utenti[UsernameInseritoDalClient]["password"]:
                Priv =  utenti[UsernameInseritoDalClient]["privilegi"]
                return jsonify({"Esito": "000", "Msg": "Utente registrato", "Privilegio": Priv}), 200
            else:
                return jsonify({"Esito": "001", "Msg": "Credenziali errate"}), 200
        else:
            return jsonify({"Esito": "001", "Msg": "Credenziali errate"}), 200
    else:
        return jsonify({"Esito": "002", "Msg": "Formato richiesta errato"}), 200
...
api.run(host="127.0.0.1", ssl_context ="adhoc", port=8080)
